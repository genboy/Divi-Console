...
function DC_theme_customizer( $wp_customize ){
    ...
    /* Custom panels
    // add panels
    $wp_customize->add_panel('DC_elements', array(
        'title'    => __('DC Elements', $themename ),
        'priority' => 2,
    ));

    // add sections
    $wp_customize->add_section('DC_elements_header', array(
        'title'    => __('Header', $themename ),
        'panel'  => 'DC_elements',
		'priority' => 20,
    ));
    $wp_customize->add_section('DC_elements_sidebar', array(
        'title'    => __('Sidebar', $themename ),
        'panel'  => 'DC_elements',
		'priority' => 20,
    ));
    ...
    // header add options
    $wp_customize->add_setting( 'DC_elements_header_secondary_display' , array(
		'default' => 'hide',
		'sanitize_callback' => 'DC_sanitize_default',
    ));
    $wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'DC_elements_header_secondary_display', array(
            	'label'          => __( 'Display Secondary bar', $themename ),
            	'section'        => 'DC_elements_header',
            	'settings'       => 'DC_elements_header_secondary_display',
            	'type'           => 'select',
    ...
    // sidebar add options
    $wp_customize->add_setting( 'DC_elements_sidebar_display' , array(
		'default' => 'right',
		'sanitize_callback' => 'DC_sanitize_default',
    ));
    $wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'DC_elements_sidebar_display', array(
            	'label'          => __( 'Sidebar position', $themename ),
            	'section'        => 'DC_elements_sidebar',
            	'settings'       => 'DC_elements_sidebar_display',
            	'type'           => 'select',
...
add_action( 'customize_register', 'DC_theme_customizer' );


// add frontend
function DC_customize_adaptive(){
    // start output css ?>
    <style>
...
add_action( 'wp_head' , 'DC_customize_adaptive' );

// default sanitize function
function DC_sanitize_default($obj){
    //.. global sanitizer
    return $obj;
}
